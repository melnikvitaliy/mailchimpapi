package com.initflow.ecwid.maleorang.util
    ...
    val Class<out MailchimpObject>.mailchimpObjectFields: Map<String, Field>
        get() = fieldsCache.get(this, {
            Collections.unmodifiableMap(LinkedHashMap<String, Field>().apply {
                for (f in getAnnotatedFields(com.initflow.ecwid.maleorang.annotation.Field::class.java)) {
                    var name = f.getAnnotation(com.initflow.ecwid.maleorang.annotation.Field::class.java).name
                    if (name.isEmpty()) {
                        name = f.name
                    }
